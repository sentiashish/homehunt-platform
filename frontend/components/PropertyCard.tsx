...
import { isPropertySaved, saveProperty } from '@/lib/wishlist'
...
export default function PropertyCard({ property, variant = 'grid' }: PropertyCardProps) {
  ...
  useEffect(() => {
    ...
    syncSavedState()
    window.addEventListener('wishlist-changed', syncSavedState)
    window.addEventListener('storage', syncSavedState)

    return () => {
      window.removeEventListener('wishlist-changed', syncSavedState)
      ...
      <>
        ...
        >
          <Link href={`/properties/${property.id}`} className="absolute inset-0 z-0">
            ...
            <div className="absolute inset-0 p-6 flex flex-col justify-end">
              <motion.div initial={{ opacity: 0, y: 10 }} whileHover={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                ...
                <div className="flex items-center text-white group-hover:text-accent transition-colors font-semibold text-sm">
          ...
          <button
            ...
            className="absolute top-4 left-4 z-20 rounded-full bg-black/50 text-white p-2 backdrop-blur-sm hover:bg-black/70"
          ...
          <div className="netflix-scale bg-card rounded-2xl overflow-hidden card-elevation h-full flex flex-col border border-gray-100 dark:border-gray-900">
            <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
              ...
              <button
                ...
                aria-label="Save property"
              >
                <Heart className={`w-4 h-4 transition-colors ${saved ? 'fill-red-500 text-red-500' : 'text-gray-800'}`} />
              </button>
              {property.featured && (
