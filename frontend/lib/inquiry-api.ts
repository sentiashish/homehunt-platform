...
const USER_INQUIRY_HISTORY_KEY = 'userInquiryHistory'

function hasWindow() {
  return typeof window !== 'undefined'
}

function addInquiryToLocalHistory(inquiry: InquiryRecord) {
  if (!hasWindow()) return

  const current = getLocalInquiryHistory()
  const next = [inquiry, ...current].slice(0, 25)
  localStorage.setItem(USER_INQUIRY_HISTORY_KEY, JSON.stringify(next))
}

export function getLocalInquiryHistory(): InquiryRecord[] {
  if (!hasWindow()) return []

  const raw = localStorage.getItem(USER_INQUIRY_HISTORY_KEY)
  if (!raw) return []
...
export async function submitInquiry(inquiry: InquiryInput): Promise<InquiryRecord> {
  ...
  addInquiryToLocalHistory(payload.inquiry)
